...
def _raise_on_error(return_code):
    if return_code != 0:
        raise SodiumError("Call returned %s" % return_code)


def _check_len(name, buf, size, max_size=None):
    if buf is None:
        raise TypeError("%s cannot be None" % (name,))

    if max_size:
        if len(buf) < size or len(buf) > max_size:
            raise ValueError("%s must be between %d and %d bytes long" %
                             (name, size, max_size))
    else:
        if len(buf) != size:
            raise ValueError("%s must be %d byte(s) long" % (name, size))


def _check_size(name, size, min_size, max_size):
    if size < min_size or size > max_size:
        raise ValueError("%s must be between %d and %d bytes long" %
                         (name, size, max_size))


def _check_min_len(name, buf, min_size):
    if buf is None:
        raise TypeError("%s cannot be None" % (name,))

    if len(buf) < min_size:
        raise ValueError("%s must be at least %d bytes long" %
                         (name, min_size))
...
def crypto_box_seed_keypair(seed):
    _check_len('seed', seed, crypto_box_SEEDBYTES)
...
def crypto_box_beforenm(pk, sk):
    _check_len('pk', pk, crypto_box_PUBLICKEYBYTES)
    _check_len('sk', sk, crypto_box_SECRETKEYBYTES)
...
def crypto_box(msg, nonce, pk, sk):
    _check_len('nonce', nonce, crypto_box_NONCEBYTES)
    _check_len('pk', pk, crypto_box_PUBLICKEYBYTES)
    _check_len('sk', sk, crypto_box_SECRETKEYBYTES)
...
def crypto_box_afternm(msg, nonce, k):
    _check_len('nonce', nonce, crypto_box_NONCEBYTES)
    _check_len('k', k, crypto_box_BEFORENMBYTES)
...
def crypto_box_open(c, nonce, pk, sk):
    _check_min_len('c', c, crypto_box_MACBYTES)
    _check_len('nonce', nonce, crypto_box_NONCEBYTES)
    _check_len('pk', pk, crypto_box_PUBLICKEYBYTES)
    _check_len('sk', sk, crypto_box_SECRETKEYBYTES)
...
def crypto_box_seal_open(c, pk, sk):
    _check_min_len('c', c, crypto_box_SEALBYTES)
    _check_len('pk', pk, crypto_box_PUBLICKEYBYTES)
    _check_len('sk', sk, crypto_box_SECRETKEYBYTES)
...
def crypto_box_open_detached(c, mac, nonce, pk, sk):
    _check_len('mac', mac, crypto_box_MACBYTES)
    _check_len('nonce', nonce, crypto_box_NONCEBYTES)
    _check_len('pk', pk, crypto_box_PUBLICKEYBYTES)
    _check_len('sk', sk, crypto_box_SECRETKEYBYTES)
...
def crypto_secretbox(msg, nonce, k):
    _check_len('nonce', nonce, crypto_secretbox_NONCEBYTES)
    _check_len('k', k, crypto_secretbox_KEYBYTES)
...
def crypto_secretbox_open(c, nonce, k):
    _check_min_len('c', c, crypto_secretbox_MACBYTES)
    _check_len('nonce', nonce, crypto_secretbox_NONCEBYTES)
    _check_len('k', k, crypto_secretbox_KEYBYTES)
...
def crypto_secretbox_open_detached(c, mac, nonce, k):
    _check_len('mac', mac, crypto_secretbox_MACBYTES)
    _check_len('nonce', nonce, crypto_secretbox_NONCEBYTES)
    _check_len('k', k, crypto_secretbox_KEYBYTES)
...
def crypto_generichash_update(state, in_):
    _check_len('state', state, crypto_generichash_STATEBYTES)
...
def crypto_generichash_blake2b_salt_personal(
    ...
    _check_len('salt', salt, crypto_generichash_blake2b_SALTBYTES)
...
def crypto_sign_seed_keypair(seed):
    _check_len('seed', seed, crypto_sign_SEEDBYTES)
...
def crypto_sign(msg, sk):
    _check_len('sk', sk, crypto_sign_SECRETKEYBYTES)
...
def crypto_sign_open(signed_msg, pk):
    _check_min_len('signed_msg', signed_msg, crypto_sign_BYTES)
    _check_len('pk', pk, crypto_sign_PUBLICKEYBYTES)
...
def crypto_sign_verify_detached(msg, sig, pk):
    _check_len('sig', sig, crypto_sign_BYTES)
    _check_len('pk', pk, crypto_sign_PUBLICKEYBYTES)
...
def crypto_sign_ed25519_pk_to_curve25519(ed25519_pk):
    _check_len("ed25519_pk", ed25519_pk, crypto_sign_ed25519_PUBLICKEYBYTES)
...
def crypto_sign_ed25519_sk_to_curve25519(ed25519_sk):
    _check_len("ed25519_sk", ed25519_sk, crypto_sign_ed25519_SECRETKEYBYTES)
...
def crypto_sign_ed25519_sk_to_seed(sk):
    _check_len("sk", sk, crypto_sign_ed25519_SECRETKEYBYTES)
...
def crypto_stream(clen, nonce, k):
    _check_len("nonce", nonce, crypto_stream_NONCEBYTES)
    _check_len("k", k, crypto_stream_KEYBYTES)
...
def crypto_stream_xsalsa20_xor_ic(msg, nonce, ic, k):
    _check_len("nonce", nonce, crypto_stream_xsalsa20_NONCEBYTES)
    _check_len("k", k, crypto_stream_xsalsa20_KEYBYTES)
...
def crypto_stream_salsa20(clen, nonce, k):
    _check_len("nonce", nonce, crypto_stream_salsa20_NONCEBYTES)
    _check_len("k", k, crypto_stream_salsa20_KEYBYTES)
...
def crypto_core_hsalsa20(in_, k, c):
    _check_len("in_", in_, crypto_core_hsalsa20_INPUTBYTES)
    _check_len("k", k, crypto_core_hsalsa20_KEYBYTES)
    if (c is not None or
            SODIUM_VERSION < (1, 0, 9)):  # pragma: no cover
        _check_len("c", c, crypto_core_hsalsa20_CONSTBYTES)
